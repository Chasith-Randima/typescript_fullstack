...
//   };
  ...
  const handleChange = (name:string) => (e:ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
    e.preventDefault();
    setFilterValues({ ...filterValues, [name]: e.target.value });
  };
  const handleSearch = (name:string) => (e:ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
    e.preventDefault();
  ...
  const handleSubmit = async (e?:any) => {
    ...
    if (localStorage.getItem("user")) {
        ...
        if(tempData){
            userId = JSON.parse(tempData)._id;
        }
      } 
            ...
            <div className="overflow-x-auto bg-white rounded shadow dark:bg-gray-900">
              <div className="">
                ...
                <table className="w-full table-auto">
                  ...
                  <tbody>
                    {allData?.map((product:any, index:number) => {
                      return (
                        <tr
